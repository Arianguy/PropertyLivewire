<div x-data="{ paymentType: @entangle('payment_type') }">
    <div class="py-12 bg-gradient-to-b from-gray-100 to-blue-50">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                <div class="flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800">Edit Receipt #{{ $receipt->id }}</h2>
                        <p class="text-gray-600">Contract: {{ $contract->name }}</p>
                    </div>
                    <div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                <div class="flex items-start gap-2 mb-4">
                    <div class="text-blue-800 text-sm font-medium uppercase tracking-wide">CONTRACT DETAILS</div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-500 mb-1">Tenant</p>
                        <p class="font-medium text-gray-900">{{ $contract->tenant->name }}</p>
                    </div>
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-500 mb-1">Property</p>
                        <p class="font-medium text-gray-900">{{ $contract->property->name ?? 'N/A' }}</p>
                    </div>
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <p class="text-sm text-gray-500 mb-1">Contract Period</p>
                        <p class="font-medium text-gray-900">{{ $contract->cstart->format('d M Y') }} - {{ $contract->cend->format('d M Y') }}</p>
        </div>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form wire:submit="save" class="space-y-6" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Receipt Category -->
                        <div class="space-y-2">
                            <label for="receipt_category" class="block text-sm font-medium text-gray-700">Receipt Category</label>
                        ...
                        <div class="space-y-2" x-show="paymentType === 'CASH'">
                            <label for="cash_status" class="block text-sm font-medium text-gray-700">Status</label>
                            <select id="cash_status" disabled
                                class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-gray-100">
                                <option value="CLEARED" selected>Cleared</option>
                            </select>
                            <p class="text-sm text-gray-500 mt-1">Cash payments are always cleared.</p>
                        </div>
                    </div>

                    <!-- Cheque specific fields -->
                    <div class="space-y-4" x-show="paymentType === 'CHEQUE'">
                        <h4 class="font-medium text-gray-700">Cheque Details</h4>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="space-y-2">
                                <label for="cheque_no" class="block text-sm font-medium text-gray-700">Cheque Number</label>
                            ...
                            <div class="space-y-2">
                                <label for="cheque_bank" class="block text-sm font-medium text-gray-700">Cheque Bank</label>
                        ...
                        <div class="space-y-2">
                            <label for="cheque_image" class="block text-sm font-medium text-gray-700">Cheque Image</label>
                            @if($receipt->hasChequeImage())
                                <div class="mb-2 flex items-center">
                                    <img src="{{ $receipt->getChequeImageUrl() }}" alt="Cheque Image" class="h-24 object-cover rounded border border-gray-300">
                    ...
                    <div class="space-y-4" x-show="paymentType === 'ONLINE_TRANSFER'">
                        <h4 class="font-medium text-gray-700">Transfer Details</h4>
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            <label for="transfer_receipt_image" class="block text-sm font-medium text-gray-700">Transfer Receipt Image</label>
                            @if($receipt->hasTransferReceiptImage())
                                <div class="mb-2 flex items-center">
                                    <img src="{{ $receipt->getTransferReceiptImageUrl() }}" alt="Transfer Receipt" class="h-24 object-cover rounded border border-gray-300">
                                    ...
                                    <div class="ml-4">
                                        <div class="flex items-center mb-1">
                                            ...
                                            <label for="remove_transfer_image" class="ml-2 block text-sm text-gray-700">Remove current image</label>
                                        </div>
                                    </div>
                </div>
            @endif
