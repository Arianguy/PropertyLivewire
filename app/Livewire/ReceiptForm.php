    ...
    protected function rules()
    {
        ...
        foreach ($this->receipts as $index => $receipt) {
            ...
            if ($receipt['payment_type'] === 'CHEQUE') {
                ...
                $rules["receipts.$index.cheque_bank"] = 'required|string|in:' . implode(',', array_keys($this->banks));
            ...
            if ($receipt['payment_type'] === 'ONLINE_TRANSFER') {
                ...
                $rules["receipts.$index.transfer_receipt_image"] = 'required|image|max:2048';
            }
        }
        ...
        DB::transaction(function () {
            foreach ($this->receipts as $receiptData) {
                $receipt = Receipt::create([
                    ...
                    'cheque_date' => $receiptData['payment_type'] === 'CHEQUE' ? $receiptData['receipt_date'] : null,
                ...
                if ($receiptData['payment_type'] === 'CHEQUE' && $receiptData['cheque_image']) {
                    $receipt->addMedia($receiptData['cheque_image']->getRealPath())
                        ->usingName('Cheque Copy - ' . $this->contract->name)
                        ->usingFileName('cheque_' . time() . '.' . $receiptData['cheque_image']->getClientOriginalExtension())
                        ->toMediaCollection('cheque_images/' . $this->contract->name, 'public');
                }

                if ($receiptData['payment_type'] === 'ONLINE_TRANSFER' && $receiptData['transfer_receipt_image']) {
                    $receipt->addMedia($receiptData['transfer_receipt_image']->getRealPath())
                        ->usingName('Transfer Receipt - ' . $this->contract->name)
                        ->usingFileName('transfer_' . time() . '.' . $receiptData['transfer_receipt_image']->getClientOriginalExtension())
                        ->toMediaCollection('transfer_receipts/' . $this->contract->name, 'public');
                }
            }
        });
